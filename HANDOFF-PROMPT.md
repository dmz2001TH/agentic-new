...
**สถานะปัจจุบัน:** ระบบรันได้ แก้ bug หลักครบแล้ว แต่ยังมีงานต่อที่ต้องทำ
...
| **maw-js** | 3456 | Fleet Control + API | `maw-js/server.ts` |
...
maw-js/src/api/deprecated.ts        ← Mock/shim endpoints for legacy frontend
...
| ทดสอบ | ผล |
|--------|-----|
| Oracle Core HTTP (47778) | ✅ Health, Learn, Search, Stats |
| Maw API (3456) | ✅ Sessions, Stats, Auth, Reflect |
| Frontend Proxy (5173→3456) | ✅ /api ผ่าน proxy ได้ |
| Tmux session detection | ✅ แสดงแค่ real sessions |
| No ghost agents | ✅ god-oracle/gemini หายแล้ว |
| Live Terminal (PTY) | ✅ Grouped session + capture pane |
| Gemini CLI ใน tmux | ✅ แสดง Trust/Auth dialog จริง |
...
5. **Oracle Core vector search** — ตอนนี้ FTS ทำงาน แต่ vector search ขึ้น warning (ต้องตั้งค่า embedding model)
6. **Deprecated API shims** — มี mock อยู่แต่อาจต้องอัพเดทให้ตรงกับ frontend เวอร์ชันใหม่
7. **Process management** — หาวิธีรัน 3 services เป็น background daemon บน Windows (PM2 หรือ NSSM)
