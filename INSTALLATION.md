...
### สร้างโฟลเดอร์และไฟล์ identity ให้ Nexus

```powershell
# สร้างโฟลเดอร์ memory
mkdir -p C:\Agentic\ψ\agents\nexus\memory
```

สร้างไฟล์ `C:\Agentic\ψ\agents\nexus\identity.md`:
...
### ปลุก Nexus ผ่าน tmux

```powershell
# สร้าง tmux session ชื่อ "nexus" แล้วรัน gemini --yolo ข้างใน
tmux new-session -s nexus "gemini --yolo"
```

### ปลุกผ่าน start-nexus.cmd (ทางลัด)
...
### ตรวจสอบว่า Nexus ออนไลน์

```powershell
# ดู tmux sessions ที่รันอยู่
tmux list-sessions
# ควรมี "nexus" อยู่ในรายการ

# หรือดูผ่าน Dashboard
# เปิด http://localhost:5173 → Fleet → ควรมี nexus แสดงอยู่
```
...
- `maw hey nexus "ทำโน่นทำนี่"` — สั่งงานผ่าน CLI (วิ่งผ่าน port 3456)
