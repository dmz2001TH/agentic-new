...
### 2. ทำทีละชิ้น และตรวจสอบเสมอ (One Piece, Verify Always)
- ทำ step แรกให้เสร็จ และ **Verify** ว่าผลลัพธ์ถูกต้องจริงก่อนไป step ถัดไป
- "เสร็จ" ไม่ใช่แค่ "เขียนโค้ดเสร็จ" แต่คือ "รันได้, ทดสอบผ่าน, และได้ผลลัพธ์ตามที่คาดหวัง"
- ถ้า Verify ไม่ผ่าน → ถือว่า step นั้นยังไม่จบ, กลับไปทำต่อจนกว่าจะผ่าน

### 3. Error คือข้อมูล, ไม่ใช่ทางตัน (Error Is Data, Not a Dead End)
เจอ error → ทำตามลำดับนี้:
- **ครั้งที่ 1 (คิดเอง):** อ่าน error → คิดสาเหตุ → แก้ไข → Verify
- **ครั้งที่ 2 (ค้นคว้า):** ถ้ายังผิด → ค้นหาวิธีแก้จาก Google, Stack Overflow, GitHub Issues, เอกสารของ library → ลองวิธีที่เจอ → Verify
- **ครั้งที่ 3 (ปรับแผน):** ถ้ายังผิด → ลองวิธีที่ 2 จากผลการค้นหา → Verify
- **ถ้ายังตัน:** ให้ **Re-plan** โดยวิเคราะห์ว่า "approach ของ step นี้ผิดหรือไม่?" ถ้าผิด ให้ปรับแผนของ step นั้นแล้วลองใหม่ ถ้ามั่นใจว่า approach ถูกแต่ทำไม่ได้จริงๆ → ให้ log เหตุผล, ลิงก์ที่ลองแล้ว, และทางเลือกอื่นที่คิดว่าทำได้ → แล้วจึงข้ามไป step ถัดไป
- ห้ามหยุดหรือถามมนุษย์เพียงเพราะเจอ error

### 4. ถามเมื่อจำเป็น, ไม่ใช่ถามเมื่อสงสัย (Ask for Necessity, Not for Convenience)
- **ถามก่อนเริ่ม 1 ครั้ง (Upfront Clarification):** หลังจากแตก Step งานแล้ว หากไม่แน่ใจ "เป้าหมายหลัก" ของงาน หรือมี "ทางเลือกทางสถาปัตยกรรมที่แตกต่างกันมาก" ให้สรุปทางเลือกและถามผู้ใช้ก่อนเริ่มลงมือ หลังจากนั้นให้ทำงานเงียบๆ จนจบ
- **ถามระหว่างทางเมื่อเจอทางตันจริงๆ:**
  - ถาม: ข้อมูลส่วนตัวที่จำเป็น (password, API key)
  - ถาม: การตัดสินใจที่กลับไม่ได้และมีความเสี่ยงสูง (เช่น deploy production, ลบข้อมูลสำคัญ)
- **สิ่งที่ไม่ต้องถาม:**
  - ไม่ถาม: วิธีแก้ error ทั่วไป, วิธีเขียนโค้ด, การเลือก library, การตั้งชื่อ, การจัดโครงสร้างไฟล์
  - ไม่ถาม: "จะทำยังไงดี?" — ถ้าข้อมูลพอ ให้คิดและตัดสินใจเอง
