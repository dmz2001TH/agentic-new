...
## 🚀 Autonomous Workflow v2.0
### หลักการ: ทำงานเงียบ, รายงานครั้งเดียว, ขอก่อนเมื่อเสี่ยง

| ลำดับ | Phase | สิ่งที่ทำ | การตรวจสอบ (Verification) |
|---|---|---|---|
| 1 | **Decompose** | อ่านเป้าหมาย → แตกเป็น 3-7 Steps ย่อย → บันทึกลง Memory | แต่ละ Step ทำจบได้ในรอบเดียว (ไม่ใหญ่เกินไป) |
| 2 | **Clarify** | ถ้ามีจุดเข้าใจไม่ตรงหรือทางเลือกต่างกันมาก → ถาม 1 ครั้งก่อนเริ่ม | เข้าใจตรงกัน → เริ่มทำงานเงียบ |
| 3 | **Risk Gate** | Step เสี่ยงสูง (deploy / ลบข้อมูล / ยิง API เสียเงิน / เปลี่ยน config) → ขอก่อน | ไม่มี Step ไหน "สุ่มยิง" โดยไม่แจ้ง |
| 4 | **Execute + Verify** | ทำทีละ Step ตามลำดับ | Verify ทุก Step: ① รันสำเร็จ ② Output ตรง ③ ไม่มี side effect ไม่คาดคิด |
| 5 | **Fix / Research** | Error → แก้ตาม cascade 4 ขั้น | Verify หลังแก้ทุกครั้ง |
| 6 | **Re-plan** | ถ้า Approach ผิด → วิเคราะห์ → แจ้งสั้น 1 บรรทัด → ปรับแผน → ทำ Step นั้นใหม่ | แจ้งเสมอเมื่อเปลี่ยน Approach ใหญ่ (ไม่เงียบ) |
| 7 | **Report** | ทำจนครบ 100% → รายงานสรุปครั้งเดียวตอนจบ | รายงานครอบคลุม: สำเร็จ, ข้าม (ถ้ามี), เหตุผล |
...
① คิดเอง — อ่าน Error, วิเคราะห์ → แก้ → Verify
  │  ❌ ยังผิด
  ▼
② ค้นคว้า — Google / SO / Docs → ลองวิธีเจอ → Verify
  │  ❌ ยังผิด
  ▼
...
④ วิเคราะห์ Approach — สิ่งที่ทำมาทั้งหมด Approach ผิดหรือไม่?
  ...
  │
  └─ ถูกแต่ทำไม่ได้ → Log ลิงก์/สิ่งที่ลองแล้ว → ข้ามไป Step ถัดไป
```

### 📏 นิยาม Verify (3 เงื่อนไข)
Step ถือว่า **Verify ผ่าน** เมื่อครบทั้ง 3 ข้อ:
1. **รันสำเร็จ** (exit code = 0)
2. **Output ตรง** (ตรงกับที่คาดหวัง)
3. **ไม่มี side effect** (ไม่ทำลายสิ่งที่เคยใช้ได้)
