...
interface LogCardOptions {
  showScore?: boolean;
  query?: string; // GOD's Final Upgrade: Semantic Highlighting
}

// GOD's Final Upgrade: Semantic Highlighting
function highlightText(text: string, query?: string, concepts: string[] = []) {
  if (!text) return text;
  
  const qTerms = query ? query.trim().split(/\s+/).filter(k => k.length > 2) : [];
...
export function LogCard({ doc, showScore, query }: LogCardProps & LogCardOptions) {
  ...
  return (
    <Link to={`/doc/${docId}`} state={{ doc }} className={styles.card} style={cardStyle}>
      ...
      <h2 className={styles.title}>{highlightText(title, query, doc.concepts)}</h2>

      <p className={styles.preview}>{highlightText(preview, query, doc.concepts)}</p>
