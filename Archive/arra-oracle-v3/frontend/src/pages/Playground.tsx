...
// GOD's Final Upgrade: Semantic Highlighting Helper
function highlightText(text: string, query: string, concepts: string[] = []) {
  if (!text) return text;
  
  // Combine query terms and concepts for highlighting
  const qTerms = query.trim().split(/\s+/).filter(k => k.length > 2);
  const terms = Array.from(new Set([...qTerms, ...concepts]));
  ...
  return (
    <>
      {parts.map((part, i) => 
        pattern.test(part) ? <mark key={i} className={styles.highlight}>{part}</mark> : part
      )}
    </>
  );
}
...
function Column({
  title, color, data, query, uniqueIds, sharedIds, hoveredDocId, onHover, visible
}: {
  title: string;
  color: string;
    ...
    <div className={styles.column}>
      ...
      <div className={styles.columnResults}>
        {data.results.map((doc, index) => {
          ...
          const rawTitle = (doc.content || '').replace(/^---[\s\S]*?---\s*/, '').replace(/^#+\s*/gm, '').split('\n')[0]?.slice(0, 80) || doc.id;
          ...
          return (
            ...
            >
              ...
              <div className={styles.resultTitle}>
                {/* GOD's Final Upgrade: Apply Semantic Highlighting */}
                {highlightText(rawTitle, query, doc.concepts)}
              </div>
            </Link>
          );
        })}
