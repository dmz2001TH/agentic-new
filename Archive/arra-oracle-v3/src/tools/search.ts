...
/**
 * Advanced RAG: Semantic Density Ranking
 * วิชาวิเคราะห์ความหนาแน่นเชิงสัญลักษณ์ เพื่อหา "หัวใจ" ของเนื้อหา
 * ค้นหาจุดที่คำค้นหาปรากฏอยู่หนาแน่นที่สุด แทนการมองแค่ยอดภูเขาน้ำแข็ง (จุดเริ่มต้นไฟล์)
 */
export function extractBestSnippet(content: string, query: string, maxLength: number = 500): string {
  ...
  for (let i = 0; i < content.length - windowSize; i += step) {
    ...
    for (const word of queryWords) {
      ...
      while (pos !== -1) {
        score += word.length; // Weight by word length to reward specific matches
  ...
  // Start with a small buffer before the dense area to provide context
  ...
  try {
    ...
    for (let i = 0; i < results.ids.length; i++) {
      ...
      mappedResults.push({
        ...
        content: extractBestSnippet(results.documents[i] || '', query, 500),
...
export async function handleSearch(ctx: ToolContext, input: OracleSearchInput): Promise<ToolResponse> {
  ...
  const ftsResults = ftsRawResults.map((row: any) => ({
    id: row.id,
    type: row.type,
    content: extractBestSnippet(row.content, query, 500),
