...
describe("scanProjectContext", () => {
  const mawJsPath = join(import.meta.dir, "..");
  const arraOraclePath = join(import.meta.dir, "..", "..", "arra-oracle-v3");
  ...
  test("scans maw-js project and finds package.json deps", () => {
    const ctx = scanProjectContext(mawJsPath);
  ...
  test("scans arra-oracle-v3 project", () => {
    const ctx = scanProjectContext(arraOraclePath);
    expect(ctx).toContain("## Project Context");
    expect(ctx).toContain("Dependencies:");
  ...
  test("respects maxFiles limit", () => {
    const ctx = scanProjectContext(mawJsPath, 5);
    // Should be shorter than unlimited scan
    const full = scanProjectContext(mawJsPath, 100);
