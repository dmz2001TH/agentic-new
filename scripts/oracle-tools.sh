...
# --- Surgical Edit (ACI) ---
ot-edit-surgical() {
  local file="${1:?Usage: ot-edit-surgical <file> <old_text> <new_text>}"
  local old_text="$2"
  local new_text="$3"
...
ot-read-range() {
  ...
  local start="${2:?Usage: read-range <file> <start_line> <end_line>}"
  local end="${3:?Usage: read-range <file> <start_line> <end_line>}"
  if [ ! -f "$file" ]; then
...
if [[ "${BASH_SOURCE[0]}" == "${0}" ]]; then
  ...
  case "$COMMAND" in
    ...
    help|*)
      ...
      echo ""
      echo "ACI v2.0 (Supreme Coding Agent):"
      echo "  verify                           — Start Verification Loop"
      echo "  edit <file> <old> <new>          — Surgical Edit"
      echo "  summary                          — Summarize Repo"
      echo "  read-range <file> <start> <end>  — Read Line Range"
      ;;
  esac
fi
